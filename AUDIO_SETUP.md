...
1.  **Check Status**:
    Run `wpctl status` and look for the asterisk `*` next to the source. It should be the `rnnoise_source` or similar virtual sink/source, not the raw `USB Camera` hardware.
    ```bash
    wpctl status
    ```
